package com.ytzl.itrip.search.util;
...
public class SolrUtil {
    private static final String url= PropertiesUtils.get("db.properties","url");
    /**
     * 创建SolrServer对象
     *
     * 该对象有两个可以使用，都是线程安全的
     ...
     * @return
     */
    public static HttpSolrClient getSolrSeriver(){
        HttpSolrClient solor=new HttpSolrClient(url);
        return solor;
    }
    ...
    public void addDoc() throws SolrServerException, IOException {
        ...
        HttpSolrClient solr = new HttpSolrClient(url + "hotel/");
        solr.add(document);
        solr.commit();
        solr.close();
    }
    ...
    public void deleteDocumentById() throws Exception {
        //选择具体的某一个solr core
        HttpSolrClient server = new HttpSolrClient(url+"hotel/");
        //删除文档
        server.deleteById("8");
        //删除所有的索引
        //solr.deleteByQuery("*:*");
     ...
     * @throws Exception
     */
    public List<SearchHotelDetailsVO> querySolr(SearchHotelDetailsVO searchHotelDetailsVO) throws Exception{
        HttpSolrClient solrServer = new HttpSolrClient(url+"hotel/");
        ...
        //参数fq, 给query增加过滤查询条件
        query.addFilterQuery("cityId:"+searchHotelDetailsVO.getId());
        ...
        //设置分页参数
        query.setStart(0);
        query.setRows(Constants.DEFAULT_PAGE_SIZE);//每一页多少值
...
//        }
        ...
        //得到实体对象
        List<SearchHotelDetailsVO> tmpLists = response.getBeans(SearchHotelDetailsVO.class);
